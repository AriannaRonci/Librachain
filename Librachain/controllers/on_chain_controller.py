...
class OnChainController:
    ...
    def get_address_list(self, shard_address):
        result = self.counter.functions.getAddressList(shard_address).call()
        print(result)

    def add_to_dictionary(self, shard_address, contract_address, my_wallet):
        result = self.counter.functions.addToDictionary(shard_address, contract_address).transact({"from": my_wallet})
        self.w3.eth.wait_for_transaction_receipt(result)
        print(result)

    def get_balance(self, shard_address):
        result = self.counter.functions.getBalance(shard_address).call()
        print(result)
